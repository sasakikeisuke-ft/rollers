class OptionType < ActiveHash::Base
  self.data = [
    ...
    {id: 11, type: 'format', info: '漢字かなカナで登録可', code: ', format: { with: /\A[ぁ-んァ-ヶ一-龥々ー]+\z/, message: ', message_ja: '"は漢字全角ひらがなカタカナで入力してください"', message_en: '"is invalid. Input full-width characters"'},
    ...
    {id: 42, type: 'uniqueness', info: '対象モデル内での重複禁止', code: ', uniqueness: { scope: :model名_id }'},
    {id: 43, type: 'uniqueness', info: '複数のモデルでの重複禁止', code: ', uniqueness: { scope: [:model名_id, :model名_id] }'}
              ]
